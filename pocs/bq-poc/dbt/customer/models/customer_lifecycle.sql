SELECT
    ...
    FROM
        patient p
    LEFT JOIN
        visit v ON p.id = v.patient_id
    LEFT JOIN
        visit_package vp ON v.id = vp.visit_id
    LEFT JOIN
        package pkg ON vp.package_id = pkg.id
    LEFT JOIN
		visit_meta_data vm ON v.id = vm.visit_id
    LEFT JOIN
        physician_note pn ON v.requisition_id = pn.requisition_id -- this join exponentially increases the time of the query
    LEFT JOIN
		biomarker_result br ON v.id = br.visit_id
    GROUP BY
